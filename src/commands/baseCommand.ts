...
import { getExcludeGlob, getRepositoryPath, openFolder, OpenRepoOption } from '../utils';

let terminal: vscode.Terminal | undefined;
let lastActiveEditor: vscode.Uri | undefined;

function openSelected(selected: string[], type: "file" | "repo") {
	if (type === "file") {
		selected.forEach(async (filePath) => {
			let paths = [filePath];
		...
		const repoPath = selected[0];
		const [uri, openOption] = repoPath.includes(':') ? selected[0].split(':') : [repoPath, '0'];
...
export async function openTerminal(scriptPath: string, type: "file" | "repo") {
	try {
		...
		const closeSubscription = vscode.window.onDidCloseTerminal((e) => {
			if (e === terminal) {
				if (fs.existsSync(chooseFilesPath)) {
					const selected = fs.readFileSync(chooseFilesPath, 'utf-8').trim().split('\n').filter(Boolean);
